...
export class HerbicideTankMixService {

  constructor(private api: ApiService, private objectValidator: ObjectValidatorService, private dummyService: DummyService) { }

  /**
   * Fetch and returns all HerbicideTankMix objects
   */
  public async getAll(): Promise<HerbicideTankMix[]> {
    const response = await this.api.request(APIRequestMethod.GET, AppConstants.API_tankMix(), null);
    if (response.success && Array.isArray(response.response) && this.objectValidator.isHerbicideObject(response.response[0].herbicide)) {
      return response.response;
    } else {
      return[];
    }
  ...
  /**
   * Fetch and return all herbicide tank mixes associated with a given chemical treatment
   * @param id chemical_treatment_id
   */
  public async getAllForTreatment(id: number): Promise<HerbicideTankMix[]> {
    const all = await this.getAll();
    const tms = all.filter((element) => element.chemicalTreatmentId === id);
    if (tms) {
      return tms;
    } else {
      return[];
...
export class ObservationChemicalTreatmentService {

  constructor(private api: ApiService, private objectValidator: ObjectValidatorService, private dummyService: DummyService) { }

  /**
   * Fetch and returns all SpeciesObservedTreated objects
   */
  public async getAll(): Promise<SpeciesObservedTreated[]> {
    const response = await this.api.request(APIRequestMethod.GET, AppConstants.API_observationChemicallyTreated(), null);
    if (response.success && Array.isArray(response.response) && this.objectValidator.isObservationObject(response.response[0].observationObject)) {
      return response.response;
    } else {
      return[];
    }
  ...
  /**
   * Fetch and return all SpeciesObservedTreated objects associated with a given chemical treatment
   * @param id chemical_treatment_id
   */
  public async getAllForTreatment(id: number): Promise<SpeciesObservedTreated[]> {
    const all = await this.getAll();
    const items = all.filter((element) => element.chemicalTreatmentId === id);
    if (items) {
      return items;
    } else {
      return[];
