...
const phases = {
  ...
  dev: {namespace:'8ecbmv-dev'    , name: `${name}`, phase: 'dev'  , changeId:changeId, suffix: `-dev-${changeId}`  , instance: `${name}-dev-${changeId}`  , version:`${version}-${changeId}`, tag:`dev-${version}-${changeId}`, host: `${name}-8ecbmv-dev.pathfinder.gov.bc.ca/${changeId}`, apiHost:`${apiName}-8ecbmv-dev.pathfinder.gov.bc.ca/${changeId}`},
